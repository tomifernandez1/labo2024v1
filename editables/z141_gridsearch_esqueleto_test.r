...
for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {
  for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {
    for (vcp in seq(0.01, 0.1, by = 0.01)) {
      for (minbucket in seq(2, 10, by = 1)) {
        # vminsplit  minima cantidad de registros en un nodo para hacer el split
        param_basicos <- list(
        ...
        tb_grid_search <- rbindlist( 
          list( tb_grid_search, 
                list( vmax_depth, vmin_split, vcp, minbucket, ganancia_promedio) ) )
      }
    }
  }
  # escribo la tabla a disco en cada vuelta del loop mas externo
...
for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {
  for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {
    for (vcp in c(-0.5, -0.4, -0.3, -0.2, -0.1, 0)) {
      for (vmin_bucket in c(5, 10, 15, 20, 25)) {
